...
class Job {
    ...
    def setGitServerDataToJobContext(gitServerName) {
        def gitServerCrApiGroup = "gitservers"
    ...
    def runStage(stageName, context, runStageName = null) {
        script.stage(runStageName ? runStageName : stageName) {
            ...
            context.codebase.config.cloneUrl = "ssh://${context.git.autouser}@${context.git.host}:${context.git.sshPort}${context.git.repositoryRelativePath?.trim() ? context.git.repositoryRelativePath : "/" + context.git.project}"
