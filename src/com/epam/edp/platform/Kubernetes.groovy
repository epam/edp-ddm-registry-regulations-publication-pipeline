...
class Kubernetes implements Platform {
    ...
    def getJsonPathValue(object, name, jsonPath, project = null) {
        def command = "oc get ${object} ${name} -o jsonpath='{${jsonPath}}'"
        if (project)
    ...
    def getJsonValue(object, name, project = null) {
        def command = "oc get ${object} ${name} -o json"
        if (project)
            command = "${command} -n ${project}"
        return script.sh(
                script: command,
    ...
    def getImageStream(imageStreamName, crApiGroup) {
        return script.sh(
                script: "oc get cbis ${imageStreamName} --ignore-not-found=true --no-headers | awk '{print \$1}'",
                returnStdout: true
        ).trim()
    }
    ...
    def getImageStreamTags(imageStreamName, crApiGroup) {
        script.sh(
                script: "oc get cbis ${imageStreamName} -o jsonpath='{range .spec.tags[*]}{.name}{\"\\n\"}{end}'",
                returnStdout: true
        ).trim().tokenize()
    }
    ...
    def protected getTags(imageStreamName, crApiGroup) {
        return script.sh(
                script: "oc get cbis ${imageStreamName} -o jsonpath='{range .spec.tags[*]}{.name}{\" | \"}{.created}{\"\\n\"}{end}'",
                returnStdout: true
        ).trim().split('\n')
    }
    ...
    def deleteObject(objectType, objectName, force = false) {
        def command = "oc delete ${objectType} ${objectName}"
        if (force) {
    ...
    def checkObjectExists(objectType, objectName, project = null) {
        def command = "oc get ${objectType} ${objectName} --ignore-not-found=true"
        if (project)
            command = "${command} -n ${project}"
    ...
    def createProjectIfNotExist(name, edpName) {
        if (!checkObjectExists("ns", name))
            script.sh("oc create ns ${name}")
    }

    def getObjectList(objectType) {
        return script.sh(
                script: "oc get ${objectType} -o jsonpath='{.items[*].metadata.name}'",
                returnStdout: true
        ).trim().tokenize()
    }
        ...
        secretSelector.each() { secret ->
            ...
            if (secret =~ /${sharedSecretsMask}/)
                if (!checkObjectExists('secrets', newSecretName))
                    script.sh("oc get --export -o yaml secret ${secret} | " +
    ...
    def createConfigMapFromFile(cmName, project, filePath) {
        script.sh("oc create configmap ${cmName} -n ${project} --from-file=${filePath} --dry-run -o yaml | oc apply -f -")
    }
